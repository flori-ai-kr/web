...
- `next.config.ts` 에 `serverExternalPackages: ['pino']` 추가 — pino를 번들하지 않고 외부 require(동적 require·sonic-boom 해석 보호).
...
- `pino`(최신 9.x)를 **prod dependency**로 추가. `package-lock.json`은 `npm install --package-lock-only`로 동기화(로컬 pnpm + CI npm ci 혼용 규칙).
