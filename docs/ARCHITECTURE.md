...
> 최종 업데이트: 2026-06-29 | session2-qa: AI 블로그 초안 편집 서버 영속화 — `updateBlogContent` (`PUT /ai/marketing/contents/{id}`) 신규 서버 액션 추가, `activeId` 추적·저장 중 레이스 가드. 온보딩 SPECIALTIES '꽃상자' 제거. 정책 레이아웃 '홈으로' 버튼 제거. | session1-guide: 타입드 블록 기반 가이드 시스템 추가 — `lib/guide/`(types·sections·articles·toc·nav·inline·inline-render·icon-map·content/*), `components/guide/`(guide-block·guide-search·guide-toc·guide-nav·guide-nav-sheet·guide-prev-next), `/admin/guide`(인덱스 전면 재작성: 히어로+클라이언트 검색+섹션 카드)·`/admin/guide/[slug]`(신규 아티클 상세 3단 라우트, 스크롤스파이 TOC). 로그인 페이지 리디자인 + 첫 진입 환영 모달(WelcomeGuideModal) 개편(강제 선택형) | session1-cleanup: 인사이트 트렌드 탭 제거 → 경매시세·지원사업 2탭. 경매 품목 북마크(낙관적), 지원사업 카드 클릭 상세 모달·데스크탑 2단 그리드·서버 검색. `ScrapTargetType`=grant만. trend_articles·instagram_* 테이블/타입/액션 제거
...
### 이미지 로딩 최적화 (2026-06-26)

모든 이미지 표시는 `src/components/ui/image-with-skeleton.tsx`의 `ImageWithSkeleton`을 사용한다. `next/image` 직접 사용을 대체하며, 스켈레톤 쉬머(로딩 중)·페이드인(로드 완료)·에러 폴백(`ImageOff` 아이콘)을 단일 컴포넌트로 통합한다. `fill` 모드와 고정 크기(`width`/`height`) 양쪽을 지원하며, `wrapperClassName` prop으로 래퍼 `<span>` 클래스를 주입할 수 있다. 갤러리·매출·고객·커뮤니티·라이트박스(`image-lightbox.tsx`) 전반에 적용됨. 로컬 blob 프리뷰(업로드 전 즉시 표시)는 스켈레톤이 불필요해 네이티브 `<img>`를 그대로 사용한다(`sale-photo-modal.tsx`). CloudFront 1년 immutable 캐시(`Cache-Control: public, max-age=31536000, immutable`)는 `aws-infra` 레포에서 별도 관리한다(이 web 레포 변경 아님).
...
| **에러 로깅** | Discord 웹훅(민감 정보 제거, 5분 중복 제거) + pino 구조화 stdout(스택 새니타이즈·redact, api와 같은 JSON 모양) | 에러 모니터링 |
...
src/lib/logger.ts     -- reportError() -> Discord 웹훅 + pino 구조화 stdout(에러)
src/lib/log.ts        -- pino 로거(server-only, api와 같은 JSON 모양 stdout)
src/lib/log-format.ts -- 순수 포맷 헬퍼(level/timestamp/스택 새니타이즈)
src/instrumentation.ts            -- Next 16 훅: 부팅 로그 + onRequestError(서버/렌더 에러)
src/app/global-error.tsx          -- Next.js 글로벌 에러 바운더리
src/app/(admin)/admin/error.tsx   -- 어드민 라우트 에러 바운더리
```

> 구조화 로그(`lib/log.ts`)는 **컨테이너 stdout**(`docker logs`/CloudWatch)으로 나가며 api(Spring/Logstash)와 같은 JSON 형태(`@timestamp` KST·`level`·`message`·`service`)다. Discord는 알림용, stdout 로그는 수집·검색용으로 역할을 나눈다. 부팅·서버에러(onRequestError)·인증 핵심이벤트(login/logout/onboarding)를 기록하며, raw 스택 대신 `sanitizeErrorStack()` 적용 + `redact` 2차 방어로 토큰/이메일/경로 유출을 막는다. HTTP access 로그는 별도 nginx(JSON) 담당.
...
| `NEXT_PUBLIC_POSTHOG_KEY` | PostHog 공개 project API key(`phc_…`) (옵션) — 제품 분석. 미설정 시 미동작 + CSP 미추가 |
| `NEXT_PUBLIC_POSTHOG_HOST` | PostHog 클라 API 호스트(미설정 시 `us.i.posthog.com`) — EU 사용 시 명시 |
...
- **테스트 파일**: `src/lib/__tests__/`(validations, property 등), `src/lib/api/mappers/__tests__/`(DTO 매퍼 9개), `src/hooks/__tests__/`(use-infinite-list, use-quick-create), `src/components/__tests__/`(공용 UI 컴포넌트 — image-with-skeleton, domain-badge 등)
