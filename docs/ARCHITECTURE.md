...
4. **Presigned URL 직접 업로드**: Vercel Server Action 본문 4.5MB 제한(413 에러)을 우회하기 위해 브라우저가 R2에 직접 PUT한다. `createPhotoUploadTargets()` Server Action이 `photo_cards` 소유권 검증 + 메타 검증 후 presigned PUT URL 배열을 반환하면, 클라이언트(`src/lib/photo-upload.ts`)가 R2 S3 API 엔드포인트(`<bucket>.<account>.r2.cloudflarestorage.com`)로 직접 업로드한다. R2 API 키는 서버에만 존재하며, presigned URL은 300초 만료된다. R2 버킷 CORS 설정에서 브라우저 PUT origin을 허용해야 한다.
...
서버(sharp)에서 압축하면 3MB 원본이 네트워크를 타고 서버까지 간 뒤 처리된다. 클라이언트에서 미리 압축하면 네트워크 전송량이 줄고 서버 부하도 없다. EXIF 회전도 자동으로 처리된다. 3MB 초과 시 자동 압축, 카드당 최대 10장의 제한을 클라이언트에서 적용한다.

**Web Worker 비활성화**: R2 직접 업로드 전환 후 `useWebWorker: false`로 설정한다. CSP `worker-src 'self'`가 browser-image-compression이 동적으로 로드하는 외부 CDN 스크립트를 차단하기 때문에 메인 스레드 압축으로 전환했다.
