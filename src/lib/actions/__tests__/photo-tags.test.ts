...
const dto = { id: '1', name: '봄', createdAt: '2026-01-01' }
...
describe('getPhotoTags', () => {
  it('매핑해서 반환', async () => {
    ...
    expect(res[0]).toEqual({ id: '1', name: '봄', created_at: '2026-01-01' })
  })
...
describe('updatePhotoTag', () => {
  it('정상 PUT', async () => {
    mockApiFetch.mockResolvedValue(dto)
    await updatePhotoTag('1', '여름')
    expect(mockApiFetch).toHaveBeenCalledWith('/photo-tags/1', expect.objectContaining({ method: 'PUT' }))
  })

  it('잘못된 id 거부', async () => {
    await expect(updatePhotoTag('x', '여름')).rejects.toThrow('ID 형식')
  })

  it('빈 이름 거부', async () => {
    await expect(updatePhotoTag('1', '  ')).rejects.toThrow('태그 이름')
  })
})
