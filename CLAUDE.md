...
├── components/guide/     # 가이드 UI 컴포넌트 — guide-button(기능 페이지 진입 버튼), guide-block(블록 렌더러), guide-toc(우측 스크롤스파이 TOC), guide-nav(좌측 섹션 트리), guide-nav-sheet(모바일 시트), guide-back-button(?from= 돌아가기), guide-prev-next(이전/다음 아티클)
...
├── lib/guide/            # 가이드 콘텐츠 시스템 (타입드 블록 기반, MDX 미사용) — types.ts(GuideBlock·GuideArticle 타입), sections.ts(섹션 메타), articles.ts(전체 아티클 집계·조회 함수), toc.ts(TOC 추출), nav.ts(prev/next), inline.ts(인라인 파서), inline-render.tsx(JSX 변환), icon-map.tsx(lucide 키→아이콘), from-feature.ts(?from= 돌아가기 매핑), content/(start·operate·grow·community·settings).ts(실제 콘텐츠)
...
| 가이드 타입·유틸 | `lib/guide/types.ts`(GuideBlock·GuideArticle·GuideSectionWithArticles 등), `lib/guide/articles.ts`(getAllSlugs·getArticleBySlug·getSectionsWithArticles), `lib/guide/from-feature.ts`(FEATURE_TARGETS·backTarget) |
| 가이드 진입 버튼 | `components/guide/guide-button.tsx`(`GuideButton`) — 각 기능 페이지에서 `/admin/guide/<slug>?from=<slug>`로 연결하는 ghost 버튼 |
