...
│   ├── guide/           # 사용 가이드 — 인덱스(섹션별 카드 그리드) + [slug]/ 아티클 상세 (3단: 좌 nav/중 본문/우 TOC, generateStaticParams)
...
├── app/onboarding/      # 소셜 신규 가입 온보딩 (registerToken 가드) — page.tsx, onboarding-form.tsx, consent-agreements.tsx, actions.ts. Step1: 실명(ownerName, 소셜 이름 프리필)·가게명·전화번호·닉네임(프리필 제거)·이메일·지역. Step2: 나이대·알게된경로(referralSources) 필수, 관심사·가게주력 선택. **Step3(동의): 전체동의 + 필수2(이용약관·개인정보) + 선택1(마케팅) — 필수 미동의 시 가입 버튼 비활성, 동의 결과를 completeRegistration body로 전달(api user_consents 저장)**. 쿼리파라미터 `?name=` (구 `?nickname=`)으로 소셜 이름 전달
├── app/policy/          # 정책 문서 (공개 라우트) — terms·privacy·marketing/page.tsx + policy-ui.tsx(셸) + layout.tsx + legal-config.ts. 홈페이지(flori.ai.kr/policy)와 내용 동기화(legal SSOT). 동의 "보기"·푸터 링크 대상
...
├── components/guide/     # 가이드 UI 컴포넌트 — guide-block(블록 렌더러), guide-search(인덱스 클라이언트 검색·필터), guide-toc(우측 스크롤스파이 TOC), guide-nav(좌측 섹션 트리), guide-nav-sheet(모바일 시트), guide-prev-next(이전/다음 아티클)
...
├── lib/guide/            # 가이드 콘텐츠 시스템 (타입드 블록 기반, MDX 미사용) — types.ts(GuideBlock·GuideArticle 타입), sections.ts(섹션 메타), articles.ts(전체 아티클 집계·조회 함수), toc.ts(TOC 추출), nav.ts(prev/next), inline.ts(인라인 파서), inline-render.tsx(JSX 변환), icon-map.tsx(lucide 키→아이콘), content/(start·operate·grow·community·settings).ts(실제 콘텐츠)
...
- `/admin/*`·`/console/*` 경로만 인증 강제. `/login`·`/onboarding`·`/auth/*`·`/healthz`·`/policy/*` 는 공개 라우트. 정책 문서(`/policy/{terms,privacy,marketing}`)는 **이 앱이 보유**한다(가입 동의 "보기"가 admin 내부에서 표출되도록 — 구: 홈페이지 외부링크). 내용은 `flori.ai.kr/policy`와 동기화하는 legal SSOT이며, `lib/constants.ts`의 `PRIVACY_POLICY_URL`·`TERMS_URL`·`MARKETING_CONSENT_URL`은 내부 절대경로(`/policy/*`)
...
- AI 블로그 마케팅(`/admin/marketing`): 키워드·상황·메모·사진 입력 → `generateBlogDraft` (`POST /ai/marketing/blog`) → 네이버 블로그 초안(제목·단락·FAQ·해시태그) 생성·서버 저장. 초안 인라인 편집 후 **`updateBlogContent` (`PUT /ai/marketing/contents/{id}`)로 서버 영속 저장**(저장 중 재생성·히스토리 전환 차단으로 비동기 레이스 방지). 말투 설정(`saveToneProfile`), 히스토리 목록·열기(`listBlogContents`·`getBlogContent`), 삭제(`deleteBlogContent`) 지원. `lib/actions/marketing.ts` + `marketing-client.tsx` + `components/blog-draft-view.tsx`.
...
| 환경변수 검증 | `lib/env.ts` |
| 공유 라벨 상수 | `lib/constants.ts` (EXPENSE_LABELS — PAYMENT_LABELS·CHANNEL_LABELS는 id 기반 계약으로 제거됨. `HOMEPAGE_URL`(외부) + 정책 내부경로 `PRIVACY_POLICY_URL`·`TERMS_URL`·`MARKETING_CONSENT_URL`=`/policy/*` + `POLICY_VERSION`) |
...
| 가이드 타입·유틸 | `lib/guide/types.ts`(GuideBlock·GuideArticle·GuideSectionWithArticles 등), `lib/guide/articles.ts`(getAllSlugs·getArticleBySlug·getSectionsWithArticles) |
