...
├── components/layout/    # app-layout(skip-link 포함, `.app-canvas h-dvh overflow-hidden` — 헤더 아래 콘텐츠 컨테이너가 스크롤, 문서 스크롤 아님), header, sidebar, bottom-nav(하단탭 bg=sidebar 화이트), page-header, empty-state, list-page-skeleton, period-header(공통)
